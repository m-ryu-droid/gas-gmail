...
GitHub Pagesで公開する、Gmail下書き作成・送信用Webアプリです。

Googleログイン、スプレッドシートURL/IDからの読み込み、`mail_recipients` シート読み込み、テンプレート保存、差し込みプレビュー、Gmail下書き作成、Gmail送信までを行います。
送信ボタンは確認ダイアログと「送信」の入力確認を挟みます。
...
`gmail.send` はGmailから実際に送信するために使います。
...
アプリから送信した場合は `送信日時` が自動入力されます。Gmailの下書きから手動送信した場合は、必要に応じて手動で入力してください。
...
3. `読み込み` を押します。
4. テンプレートと署名を入力します。
5. まず `テスト下書き作成` または `テスト送信` を押して、自分宛の内容を確認します。
6. 下書きで止めたい場合は `未作成分の下書きを作成` を押します。
7. そのまま送る場合は `未作成分を送信` を押します。
8. 送信前に確認ダイアログが出ます。さらに `送信` と入力した場合だけ送信します。

## 件名の文字化け対策

日本語の件名はMIMEヘッダー形式でエンコードして送信・下書き作成します。
以前の版で作成した下書きの件名が文字化けしている場合は、最新版で作り直してください。
